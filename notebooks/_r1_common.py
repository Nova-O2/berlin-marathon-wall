"""Shared utilities for R1 revision analyses (Phase 11.4).

Loads the wall paper baseline dataset (n=873,334, locked for R1+R2+...) and
computes wall-paper-specific features: percentage_slowdown, hit_wall,
performance_category, age_mid.

The baseline parquet is paper-individualized (per workspace convention
established 2026-05-07): each paper has its own immutable dataset copy,
generated by reproducing the original cleaning pipeline. This ensures
revision rounds (R1, R2, ...) operate on the same data as R0 submission,
allowing direct comparison of any new analysis to original headline numbers.
"""
